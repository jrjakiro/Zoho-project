...
//function to check the queue is full or not
int arc::isqueue_full(Queue *queue)
{
    return queue->icount == queue->inumber_of_frames;
}
//function to check the queue is empty or not
int arc::isqueue_empty(Queue *queue)
{
...
//function to initialize the cache structure
void arc::initARCCache(const unsigned int iCacheSize)
{
    c = iCacheSize;
    p = 0;
    ...
    arc_queue_init(mrug, iCacheSize, MRUG);
    arc_queue_init(mru, iCacheSize, MRU);
    arc_queue_init(mfu, iCacheSize, MFU);
    arc_queue_init(mfug, iCacheSize, MFUG);
}
